 ...
 * @author Janson Leung
 */
namespace writethesky\PHPExcelReader;
...
class PHPExcelReader implements \SeekableIterator, \Countable {
	...
	private $index = 0;
        private $last_error = '';
	private $default_total = 10000000;
	/**
	...
	public function __construct($filePath, $originalFileName = false, $mimeType = false) {
		if ( ! is_readable($filePath)) {
			//throw new \Exception('PHPExcel_Reader: File (' . $filePath . ') not readable');
                        $this->last_error = 'PHPExcel_Reader: File (' . $filePath . ') not readable';
			return false;
		}
		...
		if ( ! empty($originalFileName) && ! is_scalar($originalFileName)) {
			$this->last_error = 'PHPExcel_Reader: Original file (2nd parameter) is not a string or a scalar value';
			return false;
		}
		if ( ! empty($mimeType) && ! is_scalar($mimeType)) {
			$this->last_error = 'PHPExcel_Reader: Mime type (3nd parameter) is not a string or a scalar value';
			return false;
		}
	...
	/**
         * 返回表格中的数据
         *
         * @param array  返回表格中的全部数据
         */
	private function get_all(){
		if ( ! $this->handle) {
                        $this->last_error = 'PHPExcel_Reader: No file opened';
                        return false;
                }
                $Currentindex = $this->handle->key();
		$position = $this->default_total;
	 ...
	 */
	public function seek($position = false){
		if($position === false){
			return $this->get_all();
		}
		if ( ! $this->handle) {
		...
		if ($Currentindex != $position) {
			...
			if ( ! $this->handle->valid()) {
				...
				$this->last_error = 'PHPExcel_Reader: position ' . $position . ' not found';
				return false;
			}
		}
